...
exec java -client -Xmx128m -cp ${MV_CLASSPATH} -Dmultiverse.propertyfile=${MV_COMMON}/simpleclient.props -Dmultiverse.loggername=$logname multiverse.simpleclient.SimpleClient -t ${MV_COMMON}/typenumbers.txt -e ${MV_COMMON}/simpleclient.props -a $start --count $count -f ./simpleclient.txt --seconds-between $seconds "$@"
