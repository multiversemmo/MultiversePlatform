...
exec java -client -Xmx128m -cp ${MV_CLASSPATH} -Dmultiverse.propertyfile=${MV_COMMON}/simpleclient.props -Dmultiverse.loggername=$logname multiverse.server.marshalling.Trampoline multiverse.simpleclient.SimpleClient -e ${MV_COMMON}/simpleclient.props -a $start --count $count -f ./simpleclient.txt --seconds-between $seconds -s nytsplayerclient.py $CHAR_PROPS "$@" &
